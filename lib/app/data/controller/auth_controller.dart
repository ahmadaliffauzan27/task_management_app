...
class AuthController extends GetxController {
  ...
  if (!cekUsers.exists) {
    users.doc(googleUser.email).set({
     ...
     // 'list_cari':[A,AH,AHM,AHM,AHMA,AHMAD],
    }).then((value) async {
    String temp = '';
      try {
...
Future<QuerySnapshot<Map<String,dynamic>>> getPeople() async {
  ...
  var hasil = await firestore.collection('users').where('email', whereNotIn: listFriends).get();
...
Stream<DocumentSnapshot<Map<String, dynamic>>> streamTask(String taskId){
  return firestore
  .collection('task').doc(taskId).snapshots();
}
